...
interface Message {
    ...
    text: string;
    citations?: string[];
    sourceMode?: "fileSearch" | "googleSearch" | "pro" | "none";
    proModeUsed?: boolean;
}
    ...
    const handleSend = async () => {
        ...
        try {
            ...
            const botMsg: Message = {
                ...
                proModeUsed: data.proModeUsed ?? false,
            };
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-zinc-400">
                                {proMode
                                    ? "Enhanced answers enabled"
                                    : "Powered by university knowledge base"}
                            </p>
                        </div>
                    </div>
            ...
            <div className="flex-1 overflow-y-auto px-4 py-6 space-y-4 max-w-2xl mx-auto w-full">
                {messages.map((msg, i) => (
                    ...
                    >
                        ...
                        >
                            {msg.role === "model" ? (
                                <div className="prose prose-sm prose-zinc max-w-none prose-p:my-2 prose-ul:my-2 prose-li:my-1">
                            ...
                            {msg.citations && msg.citations.length > 0 && (
                                <div className="mt-2 pt-2 border-t border-zinc-100 flex flex-wrap gap-1">
                                    {msg.citations.map((citation, citationIndex) => (
                                        <span
                                            key={citationIndex}
                                            className="inline-flex items-center gap-1 text-xs text-zinc-400 bg-zinc-50 px-2 py-0.5 rounded-full"
                                        >
                                            <BookOpen size={9} />
                                            {citation}
                                        </span>
            ...
            <div className="border-t border-zinc-100 px-4 py-3 max-w-2xl mx-auto w-full bg-[#FDFDFD]">
                <div className="flex items-center gap-2 bg-white border border-zinc-200 rounded-xl px-3 py-2 focus-within:border-zinc-400 transition-colors">
                    <input
                        ...
                        placeholder={
                            proMode
                                ? "Ask with enhanced answer mode..."
                                : "Ask about UTAR courses, admissions, fees..."
                        }
                        value={input}
