...
test("parses EVERY record on the page, not just the ones with profile links", () => {
    // 6 of the 30 FICT staff have no profile page and therefore no searchId.
    // A parser anchored on searchId drops them while still passing every
    // field-level test below. This assertion is the one that catches that.
...
test("tolerates staff who have no profile page", () => {
    const records = parseStaffCards(fixture("fict.html"));
    const withoutProfile = records.filter((r) => !r.profileUrl);

    assert.equal(withoutProfile.length, 6, "FICT has 6 staff with no profile link");
    assert.ok(withoutProfile.every((r) => r.name && r.email), "they still need name and email");
});

test("parses a card that has an administrative position", () => {
    const records = parseStaffCards(fixture("rdc.html"));
    ...
    assert.match(vp.profileUrl, /staffListDetailV2\.jsp\?searchId=\d+$/);
});
...
test("joins multiple phone numbers and does not mistake one for a division", () => {
    const records = parseStaffCards(fixture("rgo-loh.html"));
